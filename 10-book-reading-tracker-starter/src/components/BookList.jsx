...
import { selelectStatus } from "../state/filterSlice";
import {
  removeBook,
  selectBooksByStatus,
  toggleReadState,
...
const BookList = () => {
  ...
  const books = useSelector((state) =>
    selectBooksByStatus(state, currentStatus)
  );
      ...
      {books.map((book) => (
        ...
        >
          ...
          <div className="flex gap-2">
            <button
              onClick={() => dispatch(toggleReadState({ payload: book.id }))}
              className="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600"
            >
              Státusz változtatása
            </button>
            <button
              onClick={() => dispatch(removeBook({ payload: book.id }))}
